      ...
      if( count($content) ) {
        foreach( $content as $p ) {
          if( $s<$options['picasa_photo_number'] ){   
            ...
            $the_photo['image_summary'] = (string) $summary['$t'];
      ...
      if ( !empty($rss_data) ){ // Check again
        foreach ( $rss_data as $item ) {
          ...
          if( $s<$picasa_options['picasa_photo_number'] ){
            ...
            $the_photo['image_title'] = str_replace(array('.jpg', '.JPG'),'',$the_photo['image_title']);
            $the_photo['image_summary'] = $item['child']['']['summary']['0']['data'];  
 ...
 */
  function add_image($i,$css="",$pin=false){
    ...
    $summary= $this->get_photo_info($i,'image_summary');
    ...
    // ZORIN review here if you want to trouble shoot 
    $this->add('title='."'". $title ."'".' alt=zorin4'. $summary ."' "); // Careful about caps with ""
    $this->add('border="0" hspace="0" vspace="0" style="'.$css.'" '.$onContextMenu.' />'); // Override the max-width set by theme
    $this->add('<figcaption>zorin2'.$summary.'</figcaption> </figure>');
 ...
 */
  function get_link($i){
    ...
    $photosummary= $this->get_photo_info($i,'image_summary');
   ...
   // ZORIN review here if you want to trouble shoot 
    if( 'original' == $link && !empty($photourl) ){
      $this->add('<a href="' . $photourl . '" class="AlpinePhotoTiles-link" target="_blank" title=" '. $phototitle .' " alt="zorin1 '. $photosummary .' ">');
      return true;
    }elseif( ($src == $link || '1' == $link) && !empty($linkurl) ){
      $this->add('<a href="' . $linkurl . '" class="AlpinePhotoTiles-link" target="_blank" title=" '. $phototitle .' " alt="zorin1 '. $photosummary .' ">');
      return true;
    }elseif( 'link' == $link && !empty($url) ){
      $this->add('<a href="' . $url . '" class="AlpinePhotoTiles-link" title=" '. $phototitle .' " alt="zorin1 '. $photosummary .' ">'); 
      return true;
    }elseif( 'fancybox' == $link && !empty($originalurl) ){
      $light = $this->get_option( 'general_lightbox' );
      $this->add('<a href="' . $originalurl . '" class="AlpinePhotoTiles-link AlpinePhotoTiles-lightbox" title=" '. $phototitle .' " alt=zorin1" '. $photosummary .' ">'); 
      return true;
    }  
    return false;    
  }
